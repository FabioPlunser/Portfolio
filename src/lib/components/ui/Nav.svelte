<script lang="ts">
    ...
    let open = false;
    let links = [
      {text: "Home", href: "/course-selector"},
      {text: "About", href: "/files"},
      {text: "Skills", href: "/logic-solver"},
      {text: "Projects", href: "/memori"},
      {text: "Blog", href: "/memori"},
    ]
  </script>
      ...
      <div class="flex-1">
        ...
        <div class="flex-none">
          <ul class="menu menu-horizontal font-bold text-shadow px-1">
            {#each links as link}
              {#if link?.children}
                <li tabindex="-1" class="hover:text-xl">
                  <a href={link.href} >
                    {link.text}
                    <i> 
                  ...
                  <ul class="p-3 bg-base-200 font-bold hover:text-xl">
              ...
              {:else}
                {#if $page.url.pathname === link.href}
                  <li><a class="underline" href={link.href}>{link.text}</a></li>
                {:else}
                  <li class="hover:underline"><a href={link.href}>{link.text}</a></li>
                {/if}
              {/if}
            {/each}
            <li tabindex="-1">
